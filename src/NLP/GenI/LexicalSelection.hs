...
combineOne tsem lexRaw eRaw = do -- Maybe monad
    -- trace ("\n" ++ (show wt)) $
  ...
  where
    ...
    unifyParamsWithWarning (l,t) =
        if length lp /= length tp
            then croak t "Parameter length mismatch"
                     ...
                     Left msg           ->
                         croak t ("Parameter unification error: " <> msg)
                     Right (ps2, subst) ->
    ...
    unifyInterfaceUsing ifn (l,e) =
        ...
        case unifyFeat (ifn l) (pinterface e) of
            Left msg             ->
                 croak e ("Interface unification error: " <> msg)
            Right (int2, fsubst) ->
...
enrich l t = do
    -- separate into interface/anchor/named
