...
type CAT={n,np,v,vn,s,pp,c,p,cl,a,d,de,par,que,qui,dont,si,il,se}
type PERSON=[1..3] 
type GENDER={m,f}
type NUMBER={sg,pl}
...
class CanonicalSententialSubjectFinite%Que Jean aille a la montagne plait a Marie
import
	CanonicalArgument[]
declare
...
class CanonicalSententialSubjectInFinitive %Aller a la montagne plait a Marie
import 
	CanonicalArgument[]
declare
	?xSubj ?I
...
class Iobject
{
	CanonicalIobject[]
  	...
  	|CliticIobjectII[]
  	|CliticIobject3[]
% % 	|CliticLocative[] %Jean pense a Marie / y pense (pas de distinction ici) 
  	|reflexiveDative[]
 	|RelativeIobject[]
  	|CleftIobjectOne[]
 	|CleftIobjectTwo[]
...
%% grabbed from benoit's grammar: fingers crossed
	...
	?xN ?X
{
	unaryRel[]*=[arg1=X];
        <syn>{
		node xN(color=red,mark=anchor)[cat = n,top=[idx=X],bot=[det = -,pers = 3]]
	}
}
	...
	?xR ?xFoot ?xAnc ?X
{
	unaryRel[]*=[arg1=X];
	<syn>{
		node xR(color=red)[cat = n, top = [idx=X], bot = [det = +]]{
			node xAnc(color=red,mark=anchor)[cat = d]
			node xFoot(color=red,mark=foot)[cat = n,top=[det = -]]
		}
	}
}	
...
value commonnoun	%Un *garçon*, La *France*
